...
|---------|--------------|
| `/magi:help [<name> \| --list]` | Read-only quick reference. Bare invocation prints command roster (descriptions pulled live from each `skills/*/SKILL.md` frontmatter), the standard workflow diagram, subagent summary, common override flags, and a state-aware next-step hint via `detect-state.sh` (fail-soft outside a magi project). `/magi:help <name>` prints details for one command. `/magi:help --list` emits the bare command list. Always allowed in any state. |
...
Every adapter at `skills/review-plan/scripts/adapters/<cli>.sh` supports two modes:
...
```json
{
  ...
  "has_diff": true,
  "hotfix_mode": false,
  "allowed_skills": ["setup", "init", "plan", "tasks", ...],
  "disallowed_skills": {
    "go": {
...
|-------|---------------|------------|
| `help` | All (read-only reference) | any time |
| `setup` / `init` | All | once each |
| `plan` | All (warns in BOOTSTRAP) | per change |
| `tasks` | PLANNING + later | major work only |
| `review-plan` | PLANNING + later | **optional** |
| `go` | TASKS_READY + later, or HOTFIX-mode PLANNING | per work session |
| `review-code` | Any state with `has_diff=true` | **mandatory** in sprint flow |
...
claude plugin install magi@magi-workflow
```
...
- Submission to Anthropic's official `claude-plugins-official` marketplace
  for one-line install (`claude plugin install magi@claude-plugins-official`).
- Non-web domain skills (data engineering, ML, mobile, game). Architecture
  is ready: add `skills/<domain>-<name>/` and
