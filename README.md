...
./test/e2e-smoke.sh                  # 真實 CLI 端到端測試
./test/e2e-fallback.sh               # mock adapter（無 token 消耗）
```
...
- **Opus 任 Coordinator** — 規劃、派工、dispatcher 分類、審議結果收斂均需長上下文與穩定推理；副作用範圍大，採用最強模型把關
- **Sonnet 任 magi-developer** — TDD 實作以撰寫測試與產出 code 為主，需快速迭代且成本可控；Sonnet 對 code 撰寫已足夠，Opus 保留予更需推理判斷的角色
- **三家並行審議**（MAGI mode） — 不同 LLM 觀點互補，能涵蓋不同類型問題（claude 嚴謹、gemini 廣度、codex 偏實作細節）；單一模型易有盲點。MAGI 加權投票讓多家共識勝出，單家極端意見需經多數推翻方可採納
...
```bash
# Mock adapter 測試（無 token 消耗；驗證 fallback 邏輯）
./test/e2e-fallback.sh

# 狀態偵測單元測試（無 token 消耗；驗證 detect-state.sh 在 8 種 state 與 warning 情境的輸出）
./test/e2e-state.sh

# 真實 CLI 測試（每位 reviewer 執行一次 short prompt）
./test/e2e-smoke.sh
```
...
- **顯式優先** — 所有副作用較大的動作均需手動觸發；plugin 不執行隱式 commit / push、不 apply infra、不 trigger deploy。**例外**：[`/magi.yolo`](#yolo-模式無人值守) 為 opt-in 的「無人值守」模式，使用者執行該指令即視為顯式同意；yolo 在無監督下對 destructive 操作（A 類 drift 回填、root 文件修改、force push）採保守 auto-skip 並全程寫 audit log。**未執行 `/magi.yolo` 時，顯式優先守則完全適用**；兩者為獨立執行路徑，並非違反守則
